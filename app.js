...
async function genHeatmap() {
  const month    = document.getElementById('hm-month').value;
  const itemName = document.getElementById('hm-item').value;
  const wrap     = document.getElementById('hm-content');

  if (!month)    { toast('Month select karo', 'err'); return; }
  if (!itemName) { toast('Item select karo', 'err'); return; }

  wrap.innerHTML = `<div class="empty"><div class="ei">⏳</div><div class="et">Calculating...</div></div>`;

  try {
    const data = await api('getMonthlyStock', { month, itemName });
    const maxL = data.maxL || 0;
    const unit = data.unit || '';
    const days = data.days || [];

    // Color logic
    function getColor(stock, max) {
      if (max <= 0) return { bg: '#e5e7eb', text: '#6b7280' };
      const pct = stock / max * 100;
      if (stock > max) return { bg: '#7c3aed', text: '#fff' };
      if (pct >= 66)   return { bg: '#f97316', text: '#fff' };
      if (pct >= 33)   return { bg: '#eab308', text: '#1a1a1a' };
      return             { bg: '#ef4444', text: '#fff' };
    }
    ...
    const avg = Math.round(closings.reduce((a,b) => a+b, 0) / closings.length);
    const min = Math.min(...closings);
    const max = Math.max(...closings);
    const daysWithChange = days.filter(d => d.inward > 0 || d.outward > 0).length;
    ...
    let html = `
      <div class="card">
        <div class="ch" style="background:linear-gradient(90deg,#f8faff,#f0f4ff);">
          <div>
            <h2 style="font-size:13px;">📅 ${itemName}</h2>
            <span style="font-size:11px;color:var(--muted);">${monthName} &nbsp;|&nbsp; Max Level: <b style="font-family:var(--mono);color:var(--navy);">${maxL} ${unit}</b></span>
          </div>
        </div>
        <div style="padding:16px 20px;">
          ...
          <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:20px;">
            <div style="background:#f8faff;border:1px solid var(--border);border-radius:8px;padding:12px;text-align:center;">
              <div style="font-size:10px;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:.7px;">Avg Stock</div>
              <div style="font-family:var(--mono);font-size:22px;font-weight:700;color:var(--navy);margin-top:4px;">${avg}</div>
            </div>
            <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:12px;text-align:center;">
              <div style="font-size:10px;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:.7px;">Min Stock</div>
              <div style="font-family:var(--mono);font-size:22px;font-weight:700;color:#ef4444;margin-top:4px;">${min}</div>
            </div>
            <div style="background:#f5f3ff;border:1px solid #ddd6fe;border-radius:8px;padding:12px;text-align:center;">
              <div style="font-size:10px;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:.7px;">Max Stock</div>
              <div style="font-family:var(--mono);font-size:22px;font-weight:700;color:#7c3aed;margin-top:4px;">${max}</div>
            </div>
            <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:12px;text-align:center;">
              <div style="font-size:10px;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:.7px;">Active Days</div>
              <div style="font-family:var(--mono);font-size:22px;font-weight:700;color:var(--green);margin-top:4px;">${daysWithChange}</div>
            </div>
          </div>
            ...
            <div style="min-width:420px;">
              <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:5px;margin-bottom:5px;">
                ${['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(d =>
                  `<div style="text-align:center;font-size:10px;font-weight:700;color:var(--muted);padding:4px 0;">${d}</div>`
                ).join('')}
              </div>
              <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:5px;">
                ${Array(startOffset).fill(0).map(() => '<div></div>').join('')}
                ${days.map(day => {
                  const c = getColor(day.closing, maxL);
                  const d = parseInt(day.date.split('-')[2]);
                  const hasActivity = day.inward > 0 || day.outward > 0;
                  return `<div style="background:${c.bg};color:${c.text};border-radius:7px;padding:6px 4px;text-align:center;min-height:56px;display:flex;flex-direction:column;justify-content:center;align-items:center;cursor:default;position:relative;"
                    title="${day.date}&#10;Opening: ${day.opening}&#10;IN: +${day.inward}&#10;OUT: -${day.outward}&#10;Closing: ${day.closing} ${unit}">
                    <div style="font-size:10px;opacity:.7;margin-bottom:2px;">${d}</div>
                    <div style="font-family:var(--mono);font-size:13px;font-weight:700;">${day.closing}</div>
                    ${hasActivity ? `<div style="position:absolute;top:3px;right:4px;width:5px;height:5px;border-radius:50%;background:${c.text};opacity:.6;"></div>` : ''}
                  </div>`;
                }).join('')}
              </div>
            </div>
          ...
          <div style="margin-top:20px;">
            <div style="font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.8px;margin-bottom:8px;">Daily Breakdown</div>
            <div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;font-size:12px;">
              <thead><tr>
                <th style="text-align:left;padding:6px 10px;background:var(--s2);border-bottom:1.5px solid var(--border);color:var(--muted);font-size:10px;text-transform:uppercase;">Date</th>
                <th style="text-align:right;padding:6px 10px;background:var(--s2);border-bottom:1.5px solid var(--border);color:var(--muted);font-size:10px;text-transform:uppercase;">Opening</th>
                <th style="text-align:right;padding:6px 10px;background:var(--s2);border-bottom:1.5px solid var(--border);color:var(--green);font-size:10px;text-transform:uppercase;">IN</th>
                <th style="text-align:right;padding:6px 10px;background:var(--s2);border-bottom:1.5px solid var(--border);color:var(--red);font-size:10px;text-transform:uppercase;">OUT</th>
                <th style="text-align:right;padding:6px 10px;background:var(--s2);border-bottom:1.5px solid var(--border);color:var(--navy);font-size:10px;text-transform:uppercase;">Closing</th>
              </tr></thead>
              <tbody>
                ${days.filter(d => d.inward > 0 || d.outward > 0).map(day => {
                  const c = getColor(day.closing, maxL);
                  return `<tr>
                    <td style="padding:6px 10px;border-bottom:1px solid var(--border);color:var(--muted);">${fmtD(day.date)}</td>
                    <td style="padding:6px 10px;border-bottom:1px solid var(--border);text-align:right;font-family:var(--mono);">${day.opening}</td>
                    <td style="padding:6px 10px;border-bottom:1px solid var(--border);text-align:right;font-family:var(--mono);font-weight:600;color:var(--green);">${day.inward > 0 ? '+'+day.inward : '—'}</td>
                    <td style="padding:6px 10px;border-bottom:1px solid var(--border);text-align:right;font-family:var(--mono);font-weight:600;color:var(--red);">${day.outward > 0 ? '-'+day.outward : '—'}</td>
                    <td style="padding:6px 10px;border-bottom:1px solid var(--border);text-align:right;">
                      <span style="font-family:var(--mono);font-weight:700;font-size:14px;background:${c.bg};color:${c.text};padding:2px 8px;border-radius:5px;">${day.closing}</span>
                    </td>
                  </tr>`;
                }).join('')}
                ${days.filter(d => d.inward > 0 || d.outward > 0).length === 0 ?
                  `<tr><td colspan="5" style="text-align:center;padding:20px;color:var(--muted);">Is month mein koi transaction nahi hua</td></tr>` : ''}
              </tbody>
            </table></div>
          </div>
